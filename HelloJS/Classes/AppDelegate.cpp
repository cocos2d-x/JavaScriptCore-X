...
public:
	...
	bool init() {
		sprite = CCSprite::spriteWithFile("mapletree.png");
		this->addChild(sprite);
		//this->schedule(schedule_selector(TestScene::updateTest));
		
		return true;
	};
	
	void updateTest() {
		CCPoint pt = ccp(rand() % 320, rand() % 480);
		int total = 1000, i=0;
		float sum = 0.0f;
		...
		for (i=0; i < total; i++) {
			...
			sum += (ed.tv_sec - st.tv_sec) + ((ed.tv_usec - st.tv_usec) / 1000000.0f);
		}
		printf("%d\t%f (%d)\n", total, sum, i);
	}
};
